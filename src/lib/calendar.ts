 ...
 * Builds Google Calendar template URLs and .ics downloads from UTC kickoffs.
 */

export interface CalendarEventInput {
  homeTeam: string;
  awayTeam: string;
  /** UTC ISO-8601 kickoff, e.g. 2026-08-08T19:00:00Z */
  kickoffUtc: string;
  venue?: string;
  competition: string;
  /** Absolute match page URL on GoalCurrent.live */
  matchPageUrl: string;
  broadcaster?: string;
  /** Default 105 min (90 + half-time allowance) */
...
/** Google Calendar / ICS UTC stamp: YYYYMMDDTHHMMSSZ */
...
export function buildGoogleCalendarDates(input: CalendarEventInput): string {
  ...
  const end = eventEndUtc(start, input.durationMinutes ?? DEFAULT_DURATION_MINUTES);
...
export function buildIcsFilename(input: CalendarEventInput): string {
  const date = formatUtcCalendarStamp(parseKickoffUtc(input.kickoffUtc)).slice(0, 8);
  return `${slugPart(input.homeTeam)}-vs-${slugPart(input.awayTeam)}-${date}.ics`;
}

export function buildIcsBlob(input: CalendarEventInput): Blob {
  return new Blob([buildIcsContent(input)], {
    type: 'text/calendar;charset=utf-8',
  });
}

/** Client-only: trigger .ics download in the browser. */
export function downloadIcsFile(input: CalendarEventInput): void {
  if (typeof window === 'undefined' || typeof document === 'undefined') return;
  const blob = buildIcsBlob(input);
