  ...
  function fetchAndRender(container) {
    GC_API.getLive(_league).then(function(matches) {
      if (!matches || !matches.length) {
        GC_API.getByDate(_league, GC_API.today()).then(function(today) {
          container.innerHTML = buildHTML(today, true);
        });
      } else {
        container.innerHTML = buildHTML(matches, false);
      }
    }).catch(function() {
      container.innerHTML = '<div class="gc-empty">⚠️ Could not load scores.<br><button class="gc-btn gc-btn-primary" onclick="GC.draw()">Retry</button></div>';
    });
  }
