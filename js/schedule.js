...
var GC_SCHEDULE = (function () {
  ...
  /* ══ UCL FINAL ══════════════════════════════════════════ */
  var UCL_FINAL = {
    homeTeam:'PSG',      homeLogo:'https://upload.wikimedia.org/wikipedia/en/thumb/a/a7/Paris_Saint-Germain_F.C..svg/110px-Paris_Saint-Germain_F.C..svg.png',
    awayTeam:'Arsenal',  awayLogo:'https://upload.wikimedia.org/wikipedia/en/thumb/5/53/Arsenal_FC.svg/110px-Arsenal_FC.svg.png',
    ...
    scorers:[],
    tv:'BT Sport / TNT Sports (UK) · CBS Sports (USA)'
  };
  ...
  var TV = {
    'Mexico':{'uk':'ITV','us':'Fox','ca':'CTV/TSN'},'South Africa':{'uk':'ITV','us':'Fox','ca':'TSN'},
    'South Korea':{'uk':'ITV','us':'FS1','ca':'TSN'},'Czechia':{'uk':'ITV','us':'FS1','ca':'TSN'},
    'Canada':{'uk':'BBC','us':'Fox','ca':'CTV/TSN'},'Bosnia & Herz.':{'uk':'BBC','us':'Fox','ca':'TSN'},
    'USA':{'uk':'BBC','us':'Fox','ca':'TSN'},'Paraguay':{'uk':'BBC','us':'Fox','ca':'TSN'},
    'Qatar':{'uk':'ITV','us':'FS1','ca':'TSN'},'Switzerland':{'uk':'ITV','us':'FS1','ca':'TSN'},
    'Brazil':{'uk':'BBC','us':'FS1','ca':'TSN'},'Morocco':{'uk':'BBC','us':'FS1','ca':'TSN'},
    'Haiti':{'uk':'BBC','us':'Fox','ca':'TSN'},'Scotland':{'uk':'BBC','us':'Fox','ca':'TSN'},
    'Germany':{'uk':'ITV','us':'Fox','ca':'TSN'},'Curacao':{'uk':'ITV','us':'Fox','ca':'TSN'},
    'Netherlands':{'uk':'ITV','us':'FS1','ca':'TSN'},'Japan':{'uk':'ITV','us':'FS1','ca':'TSN'},
    'Ivory Coast':{'uk':'BBC','us':'FS1','ca':'TSN'},'Ecuador':{'uk':'BBC','us':'FS1','ca':'TSN'},
    'Sweden':{'uk':'ITV','us':'Fox','ca':'TSN'},'Tunisia':{'uk':'ITV','us':'Fox','ca':'TSN'},
    'Spain':{'uk':'ITV','us':'FS1','ca':'TSN'},'Cape Verde':{'uk':'ITV','us':'FS1','ca':'TSN'},
    'Belgium':{'uk':'BBC','us':'Fox','ca':'TSN'},'Egypt':{'uk':'BBC','us':'Fox','ca':'TSN'},
    'Saudi Arabia':{'uk':'ITV','us':'FS1','ca':'TSN'},'Uruguay':{'uk':'ITV','us':'FS1','ca':'TSN'},
    'Iran':{'uk':'BBC','us':'Fox','ca':'TSN'},'New Zealand':{'uk':'BBC','us':'Fox','ca':'TSN'},
    'France':{'uk':'BBC','us':'FS1','ca':'TSN'},'Senegal':{'uk':'BBC','us':'FS1','ca':'TSN'},
    'Iraq':{'uk':'BBC','us':'Fox','ca':'TSN'},'Norway':{'uk':'BBC','us':'Fox','ca':'TSN'},
    'Argentina':{'uk':'ITV','us':'FS1','ca':'TSN'},'Algeria':{'uk':'ITV','us':'FS1','ca':'TSN'},
    'Austria':{'uk':'BBC','us':'Fox','ca':'TSN'},'Jordan':{'uk':'BBC','us':'Fox','ca':'TSN'},
    'Portugal':{'uk':'BBC','us':'FS1','ca':'TSN'},'DR Congo':{'uk':'BBC','us':'FS1','ca':'TSN'},
    'England':{'uk':'ITV','us':'Fox','ca':'TSN'},'Croatia':{'uk':'ITV','us':'Fox','ca':'TSN'},
    'Ghana':{'uk':'ITV','us':'FS1','ca':'TSN'},'Panama':{'uk':'ITV','us':'FS1','ca':'TSN'},
    'Uzbekistan':{'uk':'BBC','us':'Fox','ca':'TSN'},'Colombia':{'uk':'BBC','us':'Fox','ca':'TSN'},
    'Turkiye':{'uk':'ITV','us':'FS1','ca':'TSN'},'Australia':{'uk':'ITV','us':'FS1','ca':'TSN'}
  };
  ...
  function render(container) {
    ...
    if (_league === 'WC') { renderWC(container); return; }
    if (_league === 'UCL') { renderUCL(container); return; }
    /* ALL or PL — show unified schedule */
    renderAll(container);
  }

  function tzLabel() {
    try { var dt = new Date(); return dt.toLocaleTimeString('en-GB',{timeZoneName:'short'}).split(' ').pop() || 'Local'; } catch(e) { return 'Local'; }
  }

  function convertTime(timeStr, date) {
    try {
      ...
      var timeOut = dt.toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit',hour12:false,timeZone:deviceTz});
      var tzAbbr = dt.toLocaleTimeString('en-GB',{timeZoneName:'short',timeZone:deviceTz}).split(' ').pop();
      return timeOut + ' <span style="font-size:9px;color:#94a3b8">' + tzAbbr + '</span>';
    } catch(e) { return timeStr; }
  }

  /* ══ UNIFIED ALL-COMPETITIONS SCHEDULE ══════════════════ */
  function renderAll(container) {
    container.innerHTML =
      '<div style="padding-top:16px">' +
      '<div class="gc-hero-banner-wrap" style="height:140px;margin-bottom:18px">' +
        '<img src="https://images.unsplash.com/photo-1579952363873-27f3bade9f55?w=900&q=80" style="width:100%;height:100%;object-fit:cover" alt="Schedule">' +
        '<div class="gc-hero-banner-overlay">' +
          '<div class="gc-hero-banner-title">📅 Full Match Schedule</div>' +
          '<div class="gc-hero-banner-sub">PL · UCL Final · World Cup 2026</div>' +
        '</div>' +
      '</div>' +
  ...
  function buildAllDateBar() {
    ...
    for (var i = -3; i <= 45; i++) {
      ...
      var isSel   = iso === _date;
      var isPLFinal = iso === '2026-05-24';
      var isUCL    = iso === '2026-05-30';
      var isWCStart = iso === '2026-06-11';
      var hasMatch = iso === '2026-05-24' || iso === '2026-05-30' || WC_FIXTURES.some(function(f){ return f.date === iso; });
      html += '<button class="gc-date-btn' + (isToday?' gc-date-today':'') + (isSel?' gc-date-selected':'') + '"' +
        (!isSel && isUCL ? ' style="border-color:rgba(255,215,0,0.8);background:rgba(255,215,0,0.1)"' : '') +
        (!isSel && isPLFinal ? ' style="border-color:#37003c;background:rgba(55,0,60,0.06)"' : '') +
        (!isSel && isWCStart ? ' style="border-color:#d97706;background:rgba(217,119,6,0.06)"' : '') +
        ...
        (isPLFinal && !isSel ? '<span style="font-size:7px;color:#37003c;font-weight:800">PL</span>' : '') +
        (isUCL && !isSel ? '<span style="font-size:7px;color:#d97706;font-weight:800">UCL</span>' : '') +
        (isWCStart && !isSel ? '<span style="font-size:7px;color:#d97706;font-weight:800">WC</span>' : '') +
        '</button>';
    }
    bar.innerHTML = html;
    setTimeout(function() {
  ...
  function loadAllMatches() {
    ...
    /* Check for UCL Final date */
    if (_date === '2026-05-30') {
      el.innerHTML = renderUCLCard();
      return;
    }

    /* Check for PL Final Day */
    if (_date === '2026-05-24') {
      el.innerHTML = '<div class="gc-date-label">🏴󠁧󠁢󠁥󠁮󠁧󠁩 PL Final Day — Sunday 24 May 2026</div>';
      GC_API.getByDate('PL','2026-05-24').then(function(api) {
        ...
        el.innerHTML = '<div class="gc-date-label">🏴󠁧󠁢󠁥󠁮󠁧󠁩 PL Final Day — Sunday 24 May 2026 — 10 matches</div>' + merged.map(matchCard).join('');
      }).catch(function(){ el.innerHTML = '<div class="gc-date-label">🏴󠁧󠁢󠁥󠁮󠁧󠁩 PL Final Day — Sunday 24 May 2026 — 10 matches</div>' + PL_FINAL_DAY.map(matchCard).join(''); });
      return;
    }

    /* Check for World Cup fixtures */
    var wcMatches = WC_FIXTURES.filter(function(f){ return f.date === _date; });
    if (wcMatches.length) {
      var html = '<div class="gc-date-label">🌍 World Cup 2026 — ' + fmtDayLabel(_date) + ' — ' + wcMatches.length + ' matches</div>';
      wcMatches.forEach(function(f) {
        var tv = TV[f.home] || TV[f.away] || {uk:'BBC/ITV',us:'Fox/FS1',ca:'TSN'};
        var t = convertTime(f.time, f.date);
        html += '<div class="gc-match-card">' +
          '<div class="gc-match-meta"><span class="gc-match-league">' + esc(f.group) + '</span><span class="gc-badge gc-badge-pre">⏰ ' + t + '</span></div>' +
          ...
          '</div>' +
          '<div class="gc-match-footer" style="flex-direction:column;gap:3px"><span>🏟 ' + esc(f.venue) + '</span><span>📺 🇬🇧 ' + tv.uk + ' &nbsp;·&nbsp; 🇺🇸 ' + tv.us + ' &nbsp;·&nbsp; 🇨🇦 ' + tv.ca + '</span></div>' +
        '</div>';
      });
      el.innerHTML = html;
      return;
    ...
    /* Try live API for other PL dates */
    el.innerHTML = '<div class="gc-loading"><div class="gc-spinner"></div><span>Loading...</span></div>';
    GC_API.getByDate('PL', _date).then(function(m){
      if (m && m.length) { el.innerHTML = '<div class="gc-date-label">' + fmtDayLabel(_date) + '</div>' + m.map(matchCard).join(''); }
      else { el.innerHTML = '<div class="gc-empty">📅 No matches scheduled for this date.</div>'; }
    }).catch(function(){ el.innerHTML = '<div class="gc-empty">📅 No matches scheduled for this date.</div>'; });
  }

  /* ══ UCL FINAL PAGE ══════════════════════════════════════ */
  function renderUCL(container) {
    container.innerHTML = '<div style="padding-top:16px">' + renderUCLCard() + '</div>';
  }

  function renderUCLCard() {
    var now = new Date();
    var kickoff = new Date(UCL_KO);
    ...
    return '<div class="gc-match-card" style="background:linear-gradient(135deg,rgba(26,26,46,0.08),rgba(15,52,96,0.05));border:2px solid rgba(255,215,0,0.4);padding:20px;margin-bottom:14px">' +
      '<div class="gc-match-meta"><span class="gc-match-league" style="color:#0f3460;font-weight:800">⭐ UEFA CHAMPIONS LEAGUE FINAL 2026</span>' +
      (isFuture ? '<span class="gc-badge gc-badge-pre">📅 Sat 30 May</span>' : '<span class="gc-badge gc-badge-live">LIVE</span>') +
      '</div>' +
      '<div class="gc-match-body" style="margin:14px 0">' +
        '<div class="gc-team"><img class="gc-team-logo" src="' + UCL_FINAL.homeLogo + '" alt="PSG" style="width:36px;height:36px"><span class="gc-team-name" style="font-weight:800;font-size:16px">PSG</span></div>' +
        '<div class="gc-score-wrap">' +
          (isFuture ?
            '<div style="text-align:center"><div style="font-size:13px;font-weight:700;color:#d97706">17:00 UK</div><div style="font-size:11px;color:#64748b;margin-top:4px">' + days + 'd ' + hours + 'h ' + mins + 'm</div></div>'
            : '<span class="gc-score gc-score-live">? - ?</span>') +
        '</div>' +
        '<div class="gc-team gc-team-away"><img class="gc-team-logo" src="' + UCL_FINAL.awayLogo + '" alt="Arsenal" style="width:36px;height:36px"><span class="gc-team-name" style="font-weight:800;font-size:16px;color:#9B1C1C">Arsenal</span></div>' +
      '</div>' +
      '<div class="gc-match-footer" style="flex-direction:column;gap:4px">' +
        '<span>🏟 Puskás Aréna, Budapest, Hungary</span>' +
        '<span>📺 BT Sport / TNT Sports (UK) · CBS Sports (USA)</span>' +
        '<span>🎵 The Killers Kick Off Show</span>' +
      '</div>' +
    '</div>';
  }
  ...
  function renderWC(container) {
    ...
    container.innerHTML =
      ...
      '<div class="gc-hero-banner-wrap" style="height:160px;margin-bottom:18px">' +
        ...
        '<div class="gc-hero-banner-overlay">' +
          '<div class="gc-hero-banner-title">🏆 World Cup 2026 Schedule</div>' +
          '<div class="gc-hero-banner-sub">USA · Canada · Mexico · 48 Teams · 104 Matches</div>' +
        '</div>' +
      '</div>' +
  ...
  /* ══ PL SCHEDULE (standalone PL button) ══════════════════ */
  ...
  function loadPLMatches() {
    ...
    if (_date === '2026-05-24') {
      GC_API.getByDate('PL','2026-05-24').then(function(api) {
        ...
        el.innerHTML = '<div class="gc-date-label">PL Final Day — Sunday 24 May 2026</div>' + merged.map(matchCard).join('');
      }).catch(function(){ el.innerHTML = '<div class="gc-date-label">PL Final Day — Sunday 24 May 2026</div>' + PL_FINAL_DAY.map(matchCard).join(''); });
      return;
    }
    el.innerHTML = '<div class="gc-loading"><div class="gc-spinner"></div><span>Loading...</span></div>';
  ...
  function fmtDayLabel(iso) {
    ...
    if (iso === '2026-05-30') return 'UCL Final — Saturday 30 May 2026';
    if (iso === '2026-06-11') return 'World Cup Opening Day — Thursday 11 June 2026';
  ...
  return {
    render   : render,
    setLeague: setLeague,
    _pick    : function(iso) { _date = iso; buildAllDateBar(); loadAllMatches(); },
