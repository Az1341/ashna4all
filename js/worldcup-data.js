/**
 * GoalCurrent.live — FIFA World Cup 2026
 * SINGLE SOURCE OF TRUTH.
 * Fixtures are generated from the FIFA PDFs supplied by the site owner.
 * UK/BST source times are preserved as date/bst; utc is used for visitor local timezone conversion.
 * Live API may override status/score where available.
 */
(function(){
  'use strict';
  var GROUPS = {
    "A": [
        "Mexico",
        "South Africa",
        "Korea Republic",
        "Czechia"
    ],
    "B": [
        "Canada",
        "Bosnia & Herzegovina",
        "Qatar",
        "Switzerland"
    ],
    "C": [
        "Brazil",
        "Morocco",
        "Haiti",
        "Scotland"
    ],
    "D": [
        "USA",
        "Paraguay",
        "Australia",
        "Türkiye"
    ],
    "E": [
        "Germany",
        "Curaçao",
        "Côte d'Ivoire",
        "Ecuador"
    ],
    "F": [
        "Netherlands",
        "Japan",
        "Sweden",
        "Tunisia"
    ],
    "G": [
        "Belgium",
        "Egypt",
        "IR Iran",
        "New Zealand"
    ],
    "H": [
        "Spain",
        "Cabo Verde",
        "Saudi Arabia",
        "Uruguay"
    ],
    "I": [
        "France",
        "Senegal",
        "Iraq",
        "Norway"
    ],
    "J": [
        "Argentina",
        "Algeria",
        "Austria",
        "Jordan"
    ],
    "K": [
        "Portugal",
        "Congo DR",
        "Uzbekistan",
        "Colombia"
    ],
    "L": [
        "England",
        "Croatia",
        "Ghana",
        "Panama"
    ]
};
  var FLAG = {
    "Mexico": "mx",
    "South Africa": "za",
    "Korea Republic": "kr",
    "Czechia": "cz",
    "Canada": "ca",
    "Bosnia & Herzegovina": "ba",
    "USA": "us",
    "Paraguay": "py",
    "Qatar": "qa",
    "Switzerland": "ch",
    "Brazil": "br",
    "Morocco": "ma",
    "Haiti": "ht",
    "Scotland": "gb-sct",
    "Australia": "au",
    "Türkiye": "tr",
    "Germany": "de",
    "Curaçao": "cw",
    "Netherlands": "nl",
    "Japan": "jp",
    "Côte d'Ivoire": "ci",
    "Ecuador": "ec",
    "Sweden": "se",
    "Tunisia": "tn",
    "Spain": "es",
    "Cabo Verde": "cv",
    "Belgium": "be",
    "Egypt": "eg",
    "Saudi Arabia": "sa",
    "Uruguay": "uy",
    "IR Iran": "ir",
    "New Zealand": "nz",
    "France": "fr",
    "Senegal": "sn",
    "Iraq": "iq",
    "Norway": "no",
    "Argentina": "ar",
    "Algeria": "dz",
    "Austria": "at",
    "Jordan": "jo",
    "Portugal": "pt",
    "Congo DR": "cd",
    "England": "gb-eng",
    "Croatia": "hr",
    "Ghana": "gh",
    "Panama": "pa",
    "Uzbekistan": "uz",
    "Colombia": "co"
};
  var SCHEDULE = [
    {
        "id": 1,
        "dateUK": "2026-06-11",
        "timeUK": "20:00",
        "homeCode": "MEX",
        "awayCode": "RSA",
        "home": "Mexico",
        "away": "South Africa",
        "stage": "Group Stage",
        "group": "A",
        "stadium": "Mexico City Stadium",
        "city": "Mexico City",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "mx",
        "awayFlag": "za",
        "status": "FT",
        "homeScore": 2,
        "awayScore": 0,
        "events": [
            "9' Julián Quiñones — Mexico",
            "67' Raúl Jiménez — Mexico",
            "Red cards recorded in live feed"
        ],
        "utc": "2026-06-11T19:00:00Z",
        "bst": "20:00",
        "date": "2026-06-11",
        "venue": "Mexico City Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 2,
        "dateUK": "2026-06-12",
        "timeUK": "03:00",
        "homeCode": "KOR",
        "awayCode": "CZE",
        "home": "Korea Republic",
        "away": "Czechia",
        "stage": "Group Stage",
        "group": "A",
        "stadium": "Guadalajara Stadium",
        "city": "Guadalajara",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "kr",
        "awayFlag": "cz",
        "status": "FT",
        "homeScore": 2,
        "awayScore": 1,
        "events": [
            "59' Ladislav Krejčí — Czechia",
            "67' Hwang In-beom — Korea Republic",
            "80' Oh Hyeon-gyu — Korea Republic"
        ],
        "utc": "2026-06-12T02:00:00Z",
        "bst": "03:00",
        "date": "2026-06-12",
        "venue": "Guadalajara Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 3,
        "dateUK": "2026-06-12",
        "timeUK": "20:00",
        "homeCode": "CAN",
        "awayCode": "BIH",
        "home": "Canada",
        "away": "Bosnia & Herzegovina",
        "stage": "Group Stage",
        "group": "B",
        "stadium": "Toronto Stadium",
        "city": "Toronto",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "ca",
        "awayFlag": "ba",
        "utc": "2026-06-12T19:00:00Z",
        "bst": "20:00",
        "date": "2026-06-12",
        "venue": "Toronto Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 4,
        "dateUK": "2026-06-13",
        "timeUK": "02:00",
        "homeCode": "USA",
        "awayCode": "PAR",
        "home": "USA",
        "away": "Paraguay",
        "stage": "Group Stage",
        "group": "D",
        "stadium": "Los Angeles Stadium",
        "city": "Los Angeles",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "us",
        "awayFlag": "py",
        "utc": "2026-06-13T01:00:00Z",
        "bst": "02:00",
        "date": "2026-06-13",
        "venue": "Los Angeles Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 5,
        "dateUK": "2026-06-13",
        "timeUK": "20:00",
        "homeCode": "QAT",
        "awayCode": "SUI",
        "home": "Qatar",
        "away": "Switzerland",
        "stage": "Group Stage",
        "group": "B",
        "stadium": "San Francisco Bay Area Stadium",
        "city": "San Francisco Bay Area",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "qa",
        "awayFlag": "ch",
        "utc": "2026-06-13T19:00:00Z",
        "bst": "20:00",
        "date": "2026-06-13",
        "venue": "San Francisco Bay Area Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 6,
        "dateUK": "2026-06-13",
        "timeUK": "23:00",
        "homeCode": "BRA",
        "awayCode": "MAR",
        "home": "Brazil",
        "away": "Morocco",
        "stage": "Group Stage",
        "group": "C",
        "stadium": "New York/New Jersey Stadium",
        "city": "New Jersey",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "br",
        "awayFlag": "ma",
        "utc": "2026-06-13T22:00:00Z",
        "bst": "23:00",
        "date": "2026-06-13",
        "venue": "New York/New Jersey Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 7,
        "dateUK": "2026-06-14",
        "timeUK": "02:00",
        "homeCode": "HAI",
        "awayCode": "SCO",
        "home": "Haiti",
        "away": "Scotland",
        "stage": "Group Stage",
        "group": "C",
        "stadium": "Boston Stadium",
        "city": "Boston",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "ht",
        "awayFlag": "gb-sct",
        "utc": "2026-06-14T01:00:00Z",
        "bst": "02:00",
        "date": "2026-06-14",
        "venue": "Boston Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 8,
        "dateUK": "2026-06-14",
        "timeUK": "05:00",
        "homeCode": "AUS",
        "awayCode": "TUR",
        "home": "Australia",
        "away": "Türkiye",
        "stage": "Group Stage",
        "group": "D",
        "stadium": "BC Place Vancouver",
        "city": "Vancouver",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "au",
        "awayFlag": "tr",
        "utc": "2026-06-14T04:00:00Z",
        "bst": "05:00",
        "date": "2026-06-14",
        "venue": "BC Place Vancouver",
        "ukBroadcaster": ""
    },
    {
        "id": 9,
        "dateUK": "2026-06-14",
        "timeUK": "18:00",
        "homeCode": "GER",
        "awayCode": "CUW",
        "home": "Germany",
        "away": "Curaçao",
        "stage": "Group Stage",
        "group": "E",
        "stadium": "Houston Stadium",
        "city": "Houston",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "de",
        "awayFlag": "cw",
        "utc": "2026-06-14T17:00:00Z",
        "bst": "18:00",
        "date": "2026-06-14",
        "venue": "Houston Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 10,
        "dateUK": "2026-06-14",
        "timeUK": "21:00",
        "homeCode": "NED",
        "awayCode": "JPN",
        "home": "Netherlands",
        "away": "Japan",
        "stage": "Group Stage",
        "group": "F",
        "stadium": "Dallas Stadium",
        "city": "Dallas",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "nl",
        "awayFlag": "jp",
        "utc": "2026-06-14T20:00:00Z",
        "bst": "21:00",
        "date": "2026-06-14",
        "venue": "Dallas Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 11,
        "dateUK": "2026-06-15",
        "timeUK": "00:00",
        "homeCode": "CIV",
        "awayCode": "ECU",
        "home": "Côte d'Ivoire",
        "away": "Ecuador",
        "stage": "Group Stage",
        "group": "E",
        "stadium": "Philadelphia Stadium",
        "city": "Philadelphia",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "ci",
        "awayFlag": "ec",
        "utc": "2026-06-14T23:00:00Z",
        "bst": "00:00",
        "date": "2026-06-15",
        "venue": "Philadelphia Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 12,
        "dateUK": "2026-06-15",
        "timeUK": "03:00",
        "homeCode": "SWE",
        "awayCode": "TUN",
        "home": "Sweden",
        "away": "Tunisia",
        "stage": "Group Stage",
        "group": "F",
        "stadium": "Monterrey Stadium",
        "city": "Monterrey",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "se",
        "awayFlag": "tn",
        "utc": "2026-06-15T02:00:00Z",
        "bst": "03:00",
        "date": "2026-06-15",
        "venue": "Monterrey Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 13,
        "dateUK": "2026-06-15",
        "timeUK": "17:00",
        "homeCode": "ESP",
        "awayCode": "CPV",
        "home": "Spain",
        "away": "Cabo Verde",
        "stage": "Group Stage",
        "group": "H",
        "stadium": "Atlanta Stadium",
        "city": "Atlanta",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "es",
        "awayFlag": "cv",
        "utc": "2026-06-15T16:00:00Z",
        "bst": "17:00",
        "date": "2026-06-15",
        "venue": "Atlanta Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 14,
        "dateUK": "2026-06-15",
        "timeUK": "20:00",
        "homeCode": "BEL",
        "awayCode": "EGY",
        "home": "Belgium",
        "away": "Egypt",
        "stage": "Group Stage",
        "group": "G",
        "stadium": "Seattle Stadium",
        "city": "Seattle",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "be",
        "awayFlag": "eg",
        "utc": "2026-06-15T19:00:00Z",
        "bst": "20:00",
        "date": "2026-06-15",
        "venue": "Seattle Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 15,
        "dateUK": "2026-06-15",
        "timeUK": "23:00",
        "homeCode": "KSA",
        "awayCode": "URU",
        "home": "Saudi Arabia",
        "away": "Uruguay",
        "stage": "Group Stage",
        "group": "H",
        "stadium": "Miami Stadium",
        "city": "Miami",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "sa",
        "awayFlag": "uy",
        "utc": "2026-06-15T22:00:00Z",
        "bst": "23:00",
        "date": "2026-06-15",
        "venue": "Miami Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 16,
        "dateUK": "2026-06-16",
        "timeUK": "02:00",
        "homeCode": "IRN",
        "awayCode": "NZL",
        "home": "IR Iran",
        "away": "New Zealand",
        "stage": "Group Stage",
        "group": "G",
        "stadium": "Los Angeles Stadium",
        "city": "Los Angeles",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "ir",
        "awayFlag": "nz",
        "utc": "2026-06-16T01:00:00Z",
        "bst": "02:00",
        "date": "2026-06-16",
        "venue": "Los Angeles Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 17,
        "dateUK": "2026-06-16",
        "timeUK": "20:00",
        "homeCode": "FRA",
        "awayCode": "SEN",
        "home": "France",
        "away": "Senegal",
        "stage": "Group Stage",
        "group": "I",
        "stadium": "New York/New Jersey Stadium",
        "city": "New Jersey",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "fr",
        "awayFlag": "sn",
        "utc": "2026-06-16T19:00:00Z",
        "bst": "20:00",
        "date": "2026-06-16",
        "venue": "New York/New Jersey Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 18,
        "dateUK": "2026-06-16",
        "timeUK": "23:00",
        "homeCode": "IRQ",
        "awayCode": "NOR",
        "home": "Iraq",
        "away": "Norway",
        "stage": "Group Stage",
        "group": "I",
        "stadium": "Boston Stadium",
        "city": "Boston",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "iq",
        "awayFlag": "no",
        "utc": "2026-06-16T22:00:00Z",
        "bst": "23:00",
        "date": "2026-06-16",
        "venue": "Boston Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 19,
        "dateUK": "2026-06-17",
        "timeUK": "02:00",
        "homeCode": "ARG",
        "awayCode": "ALG",
        "home": "Argentina",
        "away": "Algeria",
        "stage": "Group Stage",
        "group": "J",
        "stadium": "Kansas City Stadium",
        "city": "Kansas City",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "ar",
        "awayFlag": "dz",
        "utc": "2026-06-17T01:00:00Z",
        "bst": "02:00",
        "date": "2026-06-17",
        "venue": "Kansas City Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 20,
        "dateUK": "2026-06-17",
        "timeUK": "05:00",
        "homeCode": "AUT",
        "awayCode": "JOR",
        "home": "Austria",
        "away": "Jordan",
        "stage": "Group Stage",
        "group": "J",
        "stadium": "San Francisco Bay Area Stadium",
        "city": "San Francisco Bay Area",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "at",
        "awayFlag": "jo",
        "utc": "2026-06-17T04:00:00Z",
        "bst": "05:00",
        "date": "2026-06-17",
        "venue": "San Francisco Bay Area Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 21,
        "dateUK": "2026-06-17",
        "timeUK": "18:00",
        "homeCode": "POR",
        "awayCode": "COD",
        "home": "Portugal",
        "away": "Congo DR",
        "stage": "Group Stage",
        "group": "K",
        "stadium": "Houston Stadium",
        "city": "Houston",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "pt",
        "awayFlag": "cd",
        "utc": "2026-06-17T17:00:00Z",
        "bst": "18:00",
        "date": "2026-06-17",
        "venue": "Houston Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 22,
        "dateUK": "2026-06-17",
        "timeUK": "21:00",
        "homeCode": "ENG",
        "awayCode": "CRO",
        "home": "England",
        "away": "Croatia",
        "stage": "Group Stage",
        "group": "L",
        "stadium": "Dallas Stadium",
        "city": "Dallas",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "gb-eng",
        "awayFlag": "hr",
        "utc": "2026-06-17T20:00:00Z",
        "bst": "21:00",
        "date": "2026-06-17",
        "venue": "Dallas Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 23,
        "dateUK": "2026-06-18",
        "timeUK": "00:00",
        "homeCode": "GHA",
        "awayCode": "PAN",
        "home": "Ghana",
        "away": "Panama",
        "stage": "Group Stage",
        "group": "L",
        "stadium": "Toronto Stadium",
        "city": "Toronto",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "gh",
        "awayFlag": "pa",
        "utc": "2026-06-17T23:00:00Z",
        "bst": "00:00",
        "date": "2026-06-18",
        "venue": "Toronto Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 24,
        "dateUK": "2026-06-18",
        "timeUK": "03:00",
        "homeCode": "UZB",
        "awayCode": "COL",
        "home": "Uzbekistan",
        "away": "Colombia",
        "stage": "Group Stage",
        "group": "K",
        "stadium": "Mexico City Stadium",
        "city": "Mexico City",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "uz",
        "awayFlag": "co",
        "utc": "2026-06-18T02:00:00Z",
        "bst": "03:00",
        "date": "2026-06-18",
        "venue": "Mexico City Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 25,
        "dateUK": "2026-06-18",
        "timeUK": "17:00",
        "homeCode": "CZE",
        "awayCode": "RSA",
        "home": "Czechia",
        "away": "South Africa",
        "stage": "Group Stage",
        "group": "A",
        "stadium": "Atlanta Stadium",
        "city": "Atlanta",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "cz",
        "awayFlag": "za",
        "utc": "2026-06-18T16:00:00Z",
        "bst": "17:00",
        "date": "2026-06-18",
        "venue": "Atlanta Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 26,
        "dateUK": "2026-06-18",
        "timeUK": "20:00",
        "homeCode": "SUI",
        "awayCode": "BIH",
        "home": "Switzerland",
        "away": "Bosnia & Herzegovina",
        "stage": "Group Stage",
        "group": "B",
        "stadium": "Los Angeles Stadium",
        "city": "Los Angeles",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "ch",
        "awayFlag": "ba",
        "utc": "2026-06-18T19:00:00Z",
        "bst": "20:00",
        "date": "2026-06-18",
        "venue": "Los Angeles Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 27,
        "dateUK": "2026-06-18",
        "timeUK": "23:00",
        "homeCode": "CAN",
        "awayCode": "QAT",
        "home": "Canada",
        "away": "Qatar",
        "stage": "Group Stage",
        "group": "B",
        "stadium": "BC Place Vancouver",
        "city": "Vancouver",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "ca",
        "awayFlag": "qa",
        "utc": "2026-06-18T22:00:00Z",
        "bst": "23:00",
        "date": "2026-06-18",
        "venue": "BC Place Vancouver",
        "ukBroadcaster": ""
    },
    {
        "id": 28,
        "dateUK": "2026-06-19",
        "timeUK": "02:00",
        "homeCode": "MEX",
        "awayCode": "KOR",
        "home": "Mexico",
        "away": "Korea Republic",
        "stage": "Group Stage",
        "group": "A",
        "stadium": "Guadalajara Stadium",
        "city": "Guadalajara",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "mx",
        "awayFlag": "kr",
        "utc": "2026-06-19T01:00:00Z",
        "bst": "02:00",
        "date": "2026-06-19",
        "venue": "Guadalajara Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 29,
        "dateUK": "2026-06-19",
        "timeUK": "20:00",
        "homeCode": "USA",
        "awayCode": "AUS",
        "home": "USA",
        "away": "Australia",
        "stage": "Group Stage",
        "group": "D",
        "stadium": "Seattle Stadium",
        "city": "Seattle",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "us",
        "awayFlag": "au",
        "utc": "2026-06-19T19:00:00Z",
        "bst": "20:00",
        "date": "2026-06-19",
        "venue": "Seattle Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 30,
        "dateUK": "2026-06-19",
        "timeUK": "23:00",
        "homeCode": "SCO",
        "awayCode": "MAR",
        "home": "Scotland",
        "away": "Morocco",
        "stage": "Group Stage",
        "group": "C",
        "stadium": "Boston Stadium",
        "city": "Boston",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "gb-sct",
        "awayFlag": "ma",
        "utc": "2026-06-19T22:00:00Z",
        "bst": "23:00",
        "date": "2026-06-19",
        "venue": "Boston Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 31,
        "dateUK": "2026-06-20",
        "timeUK": "01:30",
        "homeCode": "BRA",
        "awayCode": "HAI",
        "home": "Brazil",
        "away": "Haiti",
        "stage": "Group Stage",
        "group": "C",
        "stadium": "Philadelphia Stadium",
        "city": "Philadelphia",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "br",
        "awayFlag": "ht",
        "utc": "2026-06-20T00:30:00Z",
        "bst": "01:30",
        "date": "2026-06-20",
        "venue": "Philadelphia Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 32,
        "dateUK": "2026-06-20",
        "timeUK": "04:00",
        "homeCode": "TUR",
        "awayCode": "PAR",
        "home": "Türkiye",
        "away": "Paraguay",
        "stage": "Group Stage",
        "group": "D",
        "stadium": "San Francisco Bay Area Stadium",
        "city": "San Francisco Bay Area",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "tr",
        "awayFlag": "py",
        "utc": "2026-06-20T03:00:00Z",
        "bst": "04:00",
        "date": "2026-06-20",
        "venue": "San Francisco Bay Area Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 33,
        "dateUK": "2026-06-20",
        "timeUK": "18:00",
        "homeCode": "NED",
        "awayCode": "SWE",
        "home": "Netherlands",
        "away": "Sweden",
        "stage": "Group Stage",
        "group": "F",
        "stadium": "Houston Stadium",
        "city": "Houston",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "nl",
        "awayFlag": "se",
        "utc": "2026-06-20T17:00:00Z",
        "bst": "18:00",
        "date": "2026-06-20",
        "venue": "Houston Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 34,
        "dateUK": "2026-06-20",
        "timeUK": "21:00",
        "homeCode": "GER",
        "awayCode": "CIV",
        "home": "Germany",
        "away": "Côte d'Ivoire",
        "stage": "Group Stage",
        "group": "E",
        "stadium": "Toronto Stadium",
        "city": "Toronto",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "de",
        "awayFlag": "ci",
        "utc": "2026-06-20T20:00:00Z",
        "bst": "21:00",
        "date": "2026-06-20",
        "venue": "Toronto Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 35,
        "dateUK": "2026-06-21",
        "timeUK": "01:00",
        "homeCode": "ECU",
        "awayCode": "CUW",
        "home": "Ecuador",
        "away": "Curaçao",
        "stage": "Group Stage",
        "group": "E",
        "stadium": "Kansas City Stadium",
        "city": "Kansas City",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "ec",
        "awayFlag": "cw",
        "utc": "2026-06-21T00:00:00Z",
        "bst": "01:00",
        "date": "2026-06-21",
        "venue": "Kansas City Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 36,
        "dateUK": "2026-06-21",
        "timeUK": "05:00",
        "homeCode": "TUN",
        "awayCode": "JPN",
        "home": "Tunisia",
        "away": "Japan",
        "stage": "Group Stage",
        "group": "F",
        "stadium": "Monterrey Stadium",
        "city": "Monterrey",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "tn",
        "awayFlag": "jp",
        "utc": "2026-06-21T04:00:00Z",
        "bst": "05:00",
        "date": "2026-06-21",
        "venue": "Monterrey Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 37,
        "dateUK": "2026-06-21",
        "timeUK": "17:00",
        "homeCode": "ESP",
        "awayCode": "KSA",
        "home": "Spain",
        "away": "Saudi Arabia",
        "stage": "Group Stage",
        "group": "H",
        "stadium": "Atlanta Stadium",
        "city": "Atlanta",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "es",
        "awayFlag": "sa",
        "utc": "2026-06-21T16:00:00Z",
        "bst": "17:00",
        "date": "2026-06-21",
        "venue": "Atlanta Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 38,
        "dateUK": "2026-06-21",
        "timeUK": "20:00",
        "homeCode": "BEL",
        "awayCode": "IRN",
        "home": "Belgium",
        "away": "IR Iran",
        "stage": "Group Stage",
        "group": "G",
        "stadium": "Los Angeles Stadium",
        "city": "Los Angeles",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "be",
        "awayFlag": "ir",
        "utc": "2026-06-21T19:00:00Z",
        "bst": "20:00",
        "date": "2026-06-21",
        "venue": "Los Angeles Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 39,
        "dateUK": "2026-06-21",
        "timeUK": "23:00",
        "homeCode": "URU",
        "awayCode": "CPV",
        "home": "Uruguay",
        "away": "Cabo Verde",
        "stage": "Group Stage",
        "group": "H",
        "stadium": "Miami Stadium",
        "city": "Miami",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "uy",
        "awayFlag": "cv",
        "utc": "2026-06-21T22:00:00Z",
        "bst": "23:00",
        "date": "2026-06-21",
        "venue": "Miami Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 40,
        "dateUK": "2026-06-22",
        "timeUK": "02:00",
        "homeCode": "NZL",
        "awayCode": "EGY",
        "home": "New Zealand",
        "away": "Egypt",
        "stage": "Group Stage",
        "group": "G",
        "stadium": "BC Place Vancouver",
        "city": "Vancouver",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "nz",
        "awayFlag": "eg",
        "utc": "2026-06-22T01:00:00Z",
        "bst": "02:00",
        "date": "2026-06-22",
        "venue": "BC Place Vancouver",
        "ukBroadcaster": ""
    },
    {
        "id": 41,
        "dateUK": "2026-06-22",
        "timeUK": "18:00",
        "homeCode": "ARG",
        "awayCode": "AUT",
        "home": "Argentina",
        "away": "Austria",
        "stage": "Group Stage",
        "group": "J",
        "stadium": "Dallas Stadium",
        "city": "Dallas",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "ar",
        "awayFlag": "at",
        "utc": "2026-06-22T17:00:00Z",
        "bst": "18:00",
        "date": "2026-06-22",
        "venue": "Dallas Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 42,
        "dateUK": "2026-06-22",
        "timeUK": "22:00",
        "homeCode": "FRA",
        "awayCode": "IRQ",
        "home": "France",
        "away": "Iraq",
        "stage": "Group Stage",
        "group": "I",
        "stadium": "Philadelphia Stadium",
        "city": "Philadelphia",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "fr",
        "awayFlag": "iq",
        "utc": "2026-06-22T21:00:00Z",
        "bst": "22:00",
        "date": "2026-06-22",
        "venue": "Philadelphia Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 43,
        "dateUK": "2026-06-23",
        "timeUK": "01:00",
        "homeCode": "NOR",
        "awayCode": "SEN",
        "home": "Norway",
        "away": "Senegal",
        "stage": "Group Stage",
        "group": "I",
        "stadium": "New York/New Jersey Stadium",
        "city": "New Jersey",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "no",
        "awayFlag": "sn",
        "utc": "2026-06-23T00:00:00Z",
        "bst": "01:00",
        "date": "2026-06-23",
        "venue": "New York/New Jersey Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 44,
        "dateUK": "2026-06-23",
        "timeUK": "04:00",
        "homeCode": "JOR",
        "awayCode": "ALG",
        "home": "Jordan",
        "away": "Algeria",
        "stage": "Group Stage",
        "group": "J",
        "stadium": "San Francisco Bay Area Stadium",
        "city": "San Francisco Bay Area",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "jo",
        "awayFlag": "dz",
        "utc": "2026-06-23T03:00:00Z",
        "bst": "04:00",
        "date": "2026-06-23",
        "venue": "San Francisco Bay Area Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 45,
        "dateUK": "2026-06-23",
        "timeUK": "18:00",
        "homeCode": "POR",
        "awayCode": "UZB",
        "home": "Portugal",
        "away": "Uzbekistan",
        "stage": "Group Stage",
        "group": "K",
        "stadium": "Houston Stadium",
        "city": "Houston",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "pt",
        "awayFlag": "uz",
        "utc": "2026-06-23T17:00:00Z",
        "bst": "18:00",
        "date": "2026-06-23",
        "venue": "Houston Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 46,
        "dateUK": "2026-06-23",
        "timeUK": "21:00",
        "homeCode": "ENG",
        "awayCode": "GHA",
        "home": "England",
        "away": "Ghana",
        "stage": "Group Stage",
        "group": "L",
        "stadium": "Boston Stadium",
        "city": "Boston",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "gb-eng",
        "awayFlag": "gh",
        "utc": "2026-06-23T20:00:00Z",
        "bst": "21:00",
        "date": "2026-06-23",
        "venue": "Boston Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 47,
        "dateUK": "2026-06-24",
        "timeUK": "00:00",
        "homeCode": "PAN",
        "awayCode": "CRO",
        "home": "Panama",
        "away": "Croatia",
        "stage": "Group Stage",
        "group": "L",
        "stadium": "Toronto Stadium",
        "city": "Toronto",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "pa",
        "awayFlag": "hr",
        "utc": "2026-06-23T23:00:00Z",
        "bst": "00:00",
        "date": "2026-06-24",
        "venue": "Toronto Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 48,
        "dateUK": "2026-06-24",
        "timeUK": "03:00",
        "homeCode": "COL",
        "awayCode": "COD",
        "home": "Colombia",
        "away": "Congo DR",
        "stage": "Group Stage",
        "group": "K",
        "stadium": "Guadalajara Stadium",
        "city": "Guadalajara",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "co",
        "awayFlag": "cd",
        "utc": "2026-06-24T02:00:00Z",
        "bst": "03:00",
        "date": "2026-06-24",
        "venue": "Guadalajara Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 49,
        "dateUK": "2026-06-24",
        "timeUK": "20:00",
        "homeCode": "SUI",
        "awayCode": "CAN",
        "home": "Switzerland",
        "away": "Canada",
        "stage": "Group Stage",
        "group": "B",
        "stadium": "BC Place Vancouver",
        "city": "Vancouver",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "ch",
        "awayFlag": "ca",
        "utc": "2026-06-24T19:00:00Z",
        "bst": "20:00",
        "date": "2026-06-24",
        "venue": "BC Place Vancouver",
        "ukBroadcaster": ""
    },
    {
        "id": 50,
        "dateUK": "2026-06-24",
        "timeUK": "20:00",
        "homeCode": "BIH",
        "awayCode": "QAT",
        "home": "Bosnia & Herzegovina",
        "away": "Qatar",
        "stage": "Group Stage",
        "group": "B",
        "stadium": "Seattle Stadium",
        "city": "Seattle",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "ba",
        "awayFlag": "qa",
        "utc": "2026-06-24T19:00:00Z",
        "bst": "20:00",
        "date": "2026-06-24",
        "venue": "Seattle Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 51,
        "dateUK": "2026-06-24",
        "timeUK": "23:00",
        "homeCode": "SCO",
        "awayCode": "BRA",
        "home": "Scotland",
        "away": "Brazil",
        "stage": "Group Stage",
        "group": "C",
        "stadium": "Miami Stadium",
        "city": "Miami",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "gb-sct",
        "awayFlag": "br",
        "utc": "2026-06-24T22:00:00Z",
        "bst": "23:00",
        "date": "2026-06-24",
        "venue": "Miami Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 52,
        "dateUK": "2026-06-24",
        "timeUK": "23:00",
        "homeCode": "MAR",
        "awayCode": "HAI",
        "home": "Morocco",
        "away": "Haiti",
        "stage": "Group Stage",
        "group": "C",
        "stadium": "Atlanta Stadium",
        "city": "Atlanta",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "ma",
        "awayFlag": "ht",
        "utc": "2026-06-24T22:00:00Z",
        "bst": "23:00",
        "date": "2026-06-24",
        "venue": "Atlanta Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 53,
        "dateUK": "2026-06-25",
        "timeUK": "02:00",
        "homeCode": "CZE",
        "awayCode": "MEX",
        "home": "Czechia",
        "away": "Mexico",
        "stage": "Group Stage",
        "group": "A",
        "stadium": "Mexico City Stadium",
        "city": "Mexico City",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "cz",
        "awayFlag": "mx",
        "utc": "2026-06-25T01:00:00Z",
        "bst": "02:00",
        "date": "2026-06-25",
        "venue": "Mexico City Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 54,
        "dateUK": "2026-06-25",
        "timeUK": "02:00",
        "homeCode": "RSA",
        "awayCode": "KOR",
        "home": "South Africa",
        "away": "Korea Republic",
        "stage": "Group Stage",
        "group": "A",
        "stadium": "Monterrey Stadium",
        "city": "Monterrey",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "za",
        "awayFlag": "kr",
        "utc": "2026-06-25T01:00:00Z",
        "bst": "02:00",
        "date": "2026-06-25",
        "venue": "Monterrey Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 55,
        "dateUK": "2026-06-25",
        "timeUK": "21:00",
        "homeCode": "CUW",
        "awayCode": "CIV",
        "home": "Curaçao",
        "away": "Côte d'Ivoire",
        "stage": "Group Stage",
        "group": "E",
        "stadium": "Philadelphia Stadium",
        "city": "Philadelphia",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "cw",
        "awayFlag": "ci",
        "utc": "2026-06-25T20:00:00Z",
        "bst": "21:00",
        "date": "2026-06-25",
        "venue": "Philadelphia Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 56,
        "dateUK": "2026-06-25",
        "timeUK": "21:00",
        "homeCode": "ECU",
        "awayCode": "GER",
        "home": "Ecuador",
        "away": "Germany",
        "stage": "Group Stage",
        "group": "E",
        "stadium": "New York/New Jersey Stadium",
        "city": "New Jersey",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "ec",
        "awayFlag": "de",
        "utc": "2026-06-25T20:00:00Z",
        "bst": "21:00",
        "date": "2026-06-25",
        "venue": "New York/New Jersey Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 57,
        "dateUK": "2026-06-26",
        "timeUK": "00:00",
        "homeCode": "JPN",
        "awayCode": "SWE",
        "home": "Japan",
        "away": "Sweden",
        "stage": "Group Stage",
        "group": "F",
        "stadium": "Dallas Stadium",
        "city": "Dallas",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "jp",
        "awayFlag": "se",
        "utc": "2026-06-25T23:00:00Z",
        "bst": "00:00",
        "date": "2026-06-26",
        "venue": "Dallas Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 58,
        "dateUK": "2026-06-26",
        "timeUK": "00:00",
        "homeCode": "TUN",
        "awayCode": "NED",
        "home": "Tunisia",
        "away": "Netherlands",
        "stage": "Group Stage",
        "group": "F",
        "stadium": "Kansas City Stadium",
        "city": "Kansas City",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "tn",
        "awayFlag": "nl",
        "utc": "2026-06-25T23:00:00Z",
        "bst": "00:00",
        "date": "2026-06-26",
        "venue": "Kansas City Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 59,
        "dateUK": "2026-06-26",
        "timeUK": "03:00",
        "homeCode": "TUR",
        "awayCode": "USA",
        "home": "Türkiye",
        "away": "USA",
        "stage": "Group Stage",
        "group": "D",
        "stadium": "Los Angeles Stadium",
        "city": "Los Angeles",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "tr",
        "awayFlag": "us",
        "utc": "2026-06-26T02:00:00Z",
        "bst": "03:00",
        "date": "2026-06-26",
        "venue": "Los Angeles Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 60,
        "dateUK": "2026-06-26",
        "timeUK": "03:00",
        "homeCode": "PAR",
        "awayCode": "AUS",
        "home": "Paraguay",
        "away": "Australia",
        "stage": "Group Stage",
        "group": "D",
        "stadium": "San Francisco Bay Area Stadium",
        "city": "San Francisco Bay Area",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "py",
        "awayFlag": "au",
        "utc": "2026-06-26T02:00:00Z",
        "bst": "03:00",
        "date": "2026-06-26",
        "venue": "San Francisco Bay Area Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 61,
        "dateUK": "2026-06-26",
        "timeUK": "20:00",
        "homeCode": "NOR",
        "awayCode": "FRA",
        "home": "Norway",
        "away": "France",
        "stage": "Group Stage",
        "group": "I",
        "stadium": "Boston Stadium",
        "city": "Boston",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "no",
        "awayFlag": "fr",
        "utc": "2026-06-26T19:00:00Z",
        "bst": "20:00",
        "date": "2026-06-26",
        "venue": "Boston Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 62,
        "dateUK": "2026-06-26",
        "timeUK": "20:00",
        "homeCode": "SEN",
        "awayCode": "IRQ",
        "home": "Senegal",
        "away": "Iraq",
        "stage": "Group Stage",
        "group": "I",
        "stadium": "Toronto Stadium",
        "city": "Toronto",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "sn",
        "awayFlag": "iq",
        "utc": "2026-06-26T19:00:00Z",
        "bst": "20:00",
        "date": "2026-06-26",
        "venue": "Toronto Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 63,
        "dateUK": "2026-06-27",
        "timeUK": "01:00",
        "homeCode": "CPV",
        "awayCode": "KSA",
        "home": "Cabo Verde",
        "away": "Saudi Arabia",
        "stage": "Group Stage",
        "group": "H",
        "stadium": "Houston Stadium",
        "city": "Houston",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "cv",
        "awayFlag": "sa",
        "utc": "2026-06-27T00:00:00Z",
        "bst": "01:00",
        "date": "2026-06-27",
        "venue": "Houston Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 64,
        "dateUK": "2026-06-27",
        "timeUK": "01:00",
        "homeCode": "URU",
        "awayCode": "ESP",
        "home": "Uruguay",
        "away": "Spain",
        "stage": "Group Stage",
        "group": "H",
        "stadium": "Guadalajara Stadium",
        "city": "Guadalajara",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "uy",
        "awayFlag": "es",
        "utc": "2026-06-27T00:00:00Z",
        "bst": "01:00",
        "date": "2026-06-27",
        "venue": "Guadalajara Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 65,
        "dateUK": "2026-06-27",
        "timeUK": "04:00",
        "homeCode": "EGY",
        "awayCode": "IRN",
        "home": "Egypt",
        "away": "IR Iran",
        "stage": "Group Stage",
        "group": "G",
        "stadium": "Seattle Stadium",
        "city": "Seattle",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "eg",
        "awayFlag": "ir",
        "utc": "2026-06-27T03:00:00Z",
        "bst": "04:00",
        "date": "2026-06-27",
        "venue": "Seattle Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 66,
        "dateUK": "2026-06-27",
        "timeUK": "04:00",
        "homeCode": "NZL",
        "awayCode": "BEL",
        "home": "New Zealand",
        "away": "Belgium",
        "stage": "Group Stage",
        "group": "G",
        "stadium": "BC Place Vancouver",
        "city": "Vancouver",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "nz",
        "awayFlag": "be",
        "utc": "2026-06-27T03:00:00Z",
        "bst": "04:00",
        "date": "2026-06-27",
        "venue": "BC Place Vancouver",
        "ukBroadcaster": ""
    },
    {
        "id": 67,
        "dateUK": "2026-06-27",
        "timeUK": "22:00",
        "homeCode": "PAN",
        "awayCode": "ENG",
        "home": "Panama",
        "away": "England",
        "stage": "Group Stage",
        "group": "L",
        "stadium": "New York/New Jersey Stadium",
        "city": "New Jersey",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "pa",
        "awayFlag": "gb-eng",
        "utc": "2026-06-27T21:00:00Z",
        "bst": "22:00",
        "date": "2026-06-27",
        "venue": "New York/New Jersey Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 68,
        "dateUK": "2026-06-27",
        "timeUK": "22:00",
        "homeCode": "CRO",
        "awayCode": "GHA",
        "home": "Croatia",
        "away": "Ghana",
        "stage": "Group Stage",
        "group": "L",
        "stadium": "Philadelphia Stadium",
        "city": "Philadelphia",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "hr",
        "awayFlag": "gh",
        "utc": "2026-06-27T21:00:00Z",
        "bst": "22:00",
        "date": "2026-06-27",
        "venue": "Philadelphia Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 69,
        "dateUK": "2026-06-28",
        "timeUK": "00:30",
        "homeCode": "COL",
        "awayCode": "POR",
        "home": "Colombia",
        "away": "Portugal",
        "stage": "Group Stage",
        "group": "K",
        "stadium": "Miami Stadium",
        "city": "Miami",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "co",
        "awayFlag": "pt",
        "utc": "2026-06-27T23:30:00Z",
        "bst": "00:30",
        "date": "2026-06-28",
        "venue": "Miami Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 70,
        "dateUK": "2026-06-28",
        "timeUK": "00:30",
        "homeCode": "COD",
        "awayCode": "UZB",
        "home": "Congo DR",
        "away": "Uzbekistan",
        "stage": "Group Stage",
        "group": "K",
        "stadium": "Atlanta Stadium",
        "city": "Atlanta",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "cd",
        "awayFlag": "uz",
        "utc": "2026-06-27T23:30:00Z",
        "bst": "00:30",
        "date": "2026-06-28",
        "venue": "Atlanta Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 71,
        "dateUK": "2026-06-28",
        "timeUK": "03:00",
        "homeCode": "ALG",
        "awayCode": "AUT",
        "home": "Algeria",
        "away": "Austria",
        "stage": "Group Stage",
        "group": "J",
        "stadium": "Kansas City Stadium",
        "city": "Kansas City",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "dz",
        "awayFlag": "at",
        "utc": "2026-06-28T02:00:00Z",
        "bst": "03:00",
        "date": "2026-06-28",
        "venue": "Kansas City Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 72,
        "dateUK": "2026-06-28",
        "timeUK": "03:00",
        "homeCode": "JOR",
        "awayCode": "ARG",
        "home": "Jordan",
        "away": "Argentina",
        "stage": "Group Stage",
        "group": "J",
        "stadium": "Dallas Stadium",
        "city": "Dallas",
        "source": "FIFA PDF UK/BST",
        "homeFlag": "jo",
        "awayFlag": "ar",
        "utc": "2026-06-28T02:00:00Z",
        "bst": "03:00",
        "date": "2026-06-28",
        "venue": "Dallas Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 73,
        "dateUK": "2026-06-28",
        "timeUK": "20:00",
        "homeCode": "2A",
        "awayCode": "2B",
        "home": "2A",
        "away": "2B",
        "stage": "Round of 32",
        "group": "—",
        "stadium": "Los Angeles Stadium",
        "city": "Los Angeles",
        "source": "FIFA PDF UK/BST",
        "utc": "2026-06-28T19:00:00Z",
        "bst": "20:00",
        "date": "2026-06-28",
        "venue": "Los Angeles Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 74,
        "dateUK": "2026-06-29",
        "timeUK": "18:00",
        "homeCode": "1C",
        "awayCode": "2F",
        "home": "1C",
        "away": "2F",
        "stage": "Round of 32",
        "group": "—",
        "stadium": "Houston Stadium",
        "city": "Houston",
        "source": "FIFA PDF UK/BST",
        "utc": "2026-06-29T17:00:00Z",
        "bst": "18:00",
        "date": "2026-06-29",
        "venue": "Houston Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 75,
        "dateUK": "2026-06-29",
        "timeUK": "21:30",
        "homeCode": "1E",
        "awayCode": "3ABCDF",
        "home": "1E",
        "away": "3ABCDF",
        "stage": "Round of 32",
        "group": "—",
        "stadium": "Boston Stadium",
        "city": "Boston",
        "source": "FIFA PDF UK/BST",
        "utc": "2026-06-29T20:30:00Z",
        "bst": "21:30",
        "date": "2026-06-29",
        "venue": "Boston Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 76,
        "dateUK": "2026-06-30",
        "timeUK": "02:00",
        "homeCode": "1F",
        "awayCode": "2C",
        "home": "1F",
        "away": "2C",
        "stage": "Round of 32",
        "group": "—",
        "stadium": "Monterrey Stadium",
        "city": "Monterrey",
        "source": "FIFA PDF UK/BST",
        "utc": "2026-06-30T01:00:00Z",
        "bst": "02:00",
        "date": "2026-06-30",
        "venue": "Monterrey Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 77,
        "dateUK": "2026-06-30",
        "timeUK": "18:00",
        "homeCode": "2E",
        "awayCode": "2I",
        "home": "2E",
        "away": "2I",
        "stage": "Round of 32",
        "group": "—",
        "stadium": "Dallas Stadium",
        "city": "Dallas",
        "source": "FIFA PDF UK/BST",
        "utc": "2026-06-30T17:00:00Z",
        "bst": "18:00",
        "date": "2026-06-30",
        "venue": "Dallas Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 78,
        "dateUK": "2026-06-30",
        "timeUK": "22:00",
        "homeCode": "1I",
        "awayCode": "3CDFGH",
        "home": "1I",
        "away": "3CDFGH",
        "stage": "Round of 32",
        "group": "—",
        "stadium": "New York/New Jersey Stadium",
        "city": "New Jersey",
        "source": "FIFA PDF UK/BST",
        "utc": "2026-06-30T21:00:00Z",
        "bst": "22:00",
        "date": "2026-06-30",
        "venue": "New York/New Jersey Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 79,
        "dateUK": "2026-07-01",
        "timeUK": "02:00",
        "homeCode": "1A",
        "awayCode": "3CEFHI",
        "home": "1A",
        "away": "3CEFHI",
        "stage": "Round of 32",
        "group": "—",
        "stadium": "Mexico City Stadium",
        "city": "Mexico City",
        "source": "FIFA PDF UK/BST",
        "utc": "2026-07-01T01:00:00Z",
        "bst": "02:00",
        "date": "2026-07-01",
        "venue": "Mexico City Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 80,
        "dateUK": "2026-07-01",
        "timeUK": "17:00",
        "homeCode": "1L",
        "awayCode": "3EHIJK",
        "home": "1L",
        "away": "3EHIJK",
        "stage": "Round of 32",
        "group": "—",
        "stadium": "Atlanta Stadium",
        "city": "Atlanta",
        "source": "FIFA PDF UK/BST",
        "utc": "2026-07-01T16:00:00Z",
        "bst": "17:00",
        "date": "2026-07-01",
        "venue": "Atlanta Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 81,
        "dateUK": "2026-07-01",
        "timeUK": "21:00",
        "homeCode": "1G",
        "awayCode": "3AEHIJ",
        "home": "1G",
        "away": "3AEHIJ",
        "stage": "Round of 32",
        "group": "—",
        "stadium": "Seattle Stadium",
        "city": "Seattle",
        "source": "FIFA PDF UK/BST",
        "utc": "2026-07-01T20:00:00Z",
        "bst": "21:00",
        "date": "2026-07-01",
        "venue": "Seattle Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 82,
        "dateUK": "2026-07-02",
        "timeUK": "01:00",
        "homeCode": "1D",
        "awayCode": "3BEFIJ",
        "home": "1D",
        "away": "3BEFIJ",
        "stage": "Round of 32",
        "group": "—",
        "stadium": "San Francisco Bay Area Stadium",
        "city": "San Francisco Bay Area",
        "source": "FIFA PDF UK/BST",
        "utc": "2026-07-02T00:00:00Z",
        "bst": "01:00",
        "date": "2026-07-02",
        "venue": "San Francisco Bay Area Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 83,
        "dateUK": "2026-07-02",
        "timeUK": "20:00",
        "homeCode": "1H",
        "awayCode": "2J",
        "home": "1H",
        "away": "2J",
        "stage": "Round of 32",
        "group": "—",
        "stadium": "Los Angeles Stadium",
        "city": "Los Angeles",
        "source": "FIFA PDF UK/BST",
        "utc": "2026-07-02T19:00:00Z",
        "bst": "20:00",
        "date": "2026-07-02",
        "venue": "Los Angeles Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 84,
        "dateUK": "2026-07-03",
        "timeUK": "00:00",
        "homeCode": "2K",
        "awayCode": "2L",
        "home": "2K",
        "away": "2L",
        "stage": "Round of 32",
        "group": "—",
        "stadium": "Toronto Stadium",
        "city": "Toronto",
        "source": "FIFA PDF UK/BST",
        "utc": "2026-07-02T23:00:00Z",
        "bst": "00:00",
        "date": "2026-07-03",
        "venue": "Toronto Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 85,
        "dateUK": "2026-07-03",
        "timeUK": "04:00",
        "homeCode": "1B",
        "awayCode": "3EFGIJ",
        "home": "1B",
        "away": "3EFGIJ",
        "stage": "Round of 32",
        "group": "—",
        "stadium": "BC Place Vancouver",
        "city": "Vancouver",
        "source": "FIFA PDF UK/BST",
        "utc": "2026-07-03T03:00:00Z",
        "bst": "04:00",
        "date": "2026-07-03",
        "venue": "BC Place Vancouver",
        "ukBroadcaster": ""
    },
    {
        "id": 86,
        "dateUK": "2026-07-03",
        "timeUK": "19:00",
        "homeCode": "2D",
        "awayCode": "2G",
        "home": "2D",
        "away": "2G",
        "stage": "Round of 32",
        "group": "—",
        "stadium": "Dallas Stadium",
        "city": "Dallas",
        "source": "FIFA PDF UK/BST",
        "utc": "2026-07-03T18:00:00Z",
        "bst": "19:00",
        "date": "2026-07-03",
        "venue": "Dallas Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 87,
        "dateUK": "2026-07-03",
        "timeUK": "23:00",
        "homeCode": "1J",
        "awayCode": "2H",
        "home": "1J",
        "away": "2H",
        "stage": "Round of 32",
        "group": "—",
        "stadium": "Miami Stadium",
        "city": "Miami",
        "source": "FIFA PDF UK/BST",
        "utc": "2026-07-03T22:00:00Z",
        "bst": "23:00",
        "date": "2026-07-03",
        "venue": "Miami Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 88,
        "dateUK": "2026-07-04",
        "timeUK": "02:30",
        "homeCode": "1K",
        "awayCode": "3DEIJL",
        "home": "1K",
        "away": "3DEIJL",
        "stage": "Round of 32",
        "group": "—",
        "stadium": "Kansas City Stadium",
        "city": "Kansas City",
        "source": "FIFA PDF UK/BST",
        "utc": "2026-07-04T01:30:00Z",
        "bst": "02:30",
        "date": "2026-07-04",
        "venue": "Kansas City Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 89,
        "dateUK": "2026-07-04",
        "timeUK": "18:00",
        "homeCode": "W73",
        "awayCode": "W75",
        "home": "W73",
        "away": "W75",
        "stage": "Round of 16",
        "group": "—",
        "stadium": "Houston Stadium",
        "city": "Houston",
        "source": "FIFA PDF UK/BST",
        "utc": "2026-07-04T17:00:00Z",
        "bst": "18:00",
        "date": "2026-07-04",
        "venue": "Houston Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 90,
        "dateUK": "2026-07-04",
        "timeUK": "22:00",
        "homeCode": "W74",
        "awayCode": "W77",
        "home": "W74",
        "away": "W77",
        "stage": "Round of 16",
        "group": "—",
        "stadium": "Philadelphia Stadium",
        "city": "Philadelphia",
        "source": "FIFA PDF UK/BST",
        "utc": "2026-07-04T21:00:00Z",
        "bst": "22:00",
        "date": "2026-07-04",
        "venue": "Philadelphia Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 91,
        "dateUK": "2026-07-05",
        "timeUK": "21:00",
        "homeCode": "W76",
        "awayCode": "W78",
        "home": "W76",
        "away": "W78",
        "stage": "Round of 16",
        "group": "—",
        "stadium": "New York/New Jersey Stadium",
        "city": "New Jersey",
        "source": "FIFA PDF UK/BST",
        "utc": "2026-07-05T20:00:00Z",
        "bst": "21:00",
        "date": "2026-07-05",
        "venue": "New York/New Jersey Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 92,
        "dateUK": "2026-07-06",
        "timeUK": "01:00",
        "homeCode": "W79",
        "awayCode": "W80",
        "home": "W79",
        "away": "W80",
        "stage": "Round of 16",
        "group": "—",
        "stadium": "Mexico City Stadium",
        "city": "Mexico City",
        "source": "FIFA PDF UK/BST",
        "utc": "2026-07-06T00:00:00Z",
        "bst": "01:00",
        "date": "2026-07-06",
        "venue": "Mexico City Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 93,
        "dateUK": "2026-07-06",
        "timeUK": "20:00",
        "homeCode": "W83",
        "awayCode": "W84",
        "home": "W83",
        "away": "W84",
        "stage": "Round of 16",
        "group": "—",
        "stadium": "Dallas Stadium",
        "city": "Dallas",
        "source": "FIFA PDF UK/BST",
        "utc": "2026-07-06T19:00:00Z",
        "bst": "20:00",
        "date": "2026-07-06",
        "venue": "Dallas Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 94,
        "dateUK": "2026-07-07",
        "timeUK": "01:00",
        "homeCode": "W81",
        "awayCode": "W82",
        "home": "W81",
        "away": "W82",
        "stage": "Round of 16",
        "group": "—",
        "stadium": "Seattle Stadium",
        "city": "Seattle",
        "source": "FIFA PDF UK/BST",
        "utc": "2026-07-07T00:00:00Z",
        "bst": "01:00",
        "date": "2026-07-07",
        "venue": "Seattle Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 95,
        "dateUK": "2026-07-07",
        "timeUK": "17:00",
        "homeCode": "W86",
        "awayCode": "W88",
        "home": "W86",
        "away": "W88",
        "stage": "Round of 16",
        "group": "—",
        "stadium": "Atlanta Stadium",
        "city": "Atlanta",
        "source": "FIFA PDF UK/BST",
        "utc": "2026-07-07T16:00:00Z",
        "bst": "17:00",
        "date": "2026-07-07",
        "venue": "Atlanta Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 96,
        "dateUK": "2026-07-07",
        "timeUK": "21:00",
        "homeCode": "W85",
        "awayCode": "W87",
        "home": "W85",
        "away": "W87",
        "stage": "Round of 16",
        "group": "—",
        "stadium": "BC Place Vancouver",
        "city": "Vancouver",
        "source": "FIFA PDF UK/BST",
        "utc": "2026-07-07T20:00:00Z",
        "bst": "21:00",
        "date": "2026-07-07",
        "venue": "BC Place Vancouver",
        "ukBroadcaster": ""
    },
    {
        "id": 97,
        "dateUK": "2026-07-09",
        "timeUK": "21:00",
        "homeCode": "W89",
        "awayCode": "W90",
        "home": "W89",
        "away": "W90",
        "stage": "Quarter-final",
        "group": "—",
        "stadium": "Boston Stadium",
        "city": "Boston",
        "source": "FIFA PDF UK/BST",
        "utc": "2026-07-09T20:00:00Z",
        "bst": "21:00",
        "date": "2026-07-09",
        "venue": "Boston Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 98,
        "dateUK": "2026-07-10",
        "timeUK": "20:00",
        "homeCode": "W93",
        "awayCode": "W94",
        "home": "W93",
        "away": "W94",
        "stage": "Quarter-final",
        "group": "—",
        "stadium": "Los Angeles Stadium",
        "city": "Los Angeles",
        "source": "FIFA PDF UK/BST",
        "utc": "2026-07-10T19:00:00Z",
        "bst": "20:00",
        "date": "2026-07-10",
        "venue": "Los Angeles Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 99,
        "dateUK": "2026-07-11",
        "timeUK": "22:00",
        "homeCode": "W91",
        "awayCode": "W92",
        "home": "W91",
        "away": "W92",
        "stage": "Quarter-final",
        "group": "—",
        "stadium": "Miami Stadium",
        "city": "Miami",
        "source": "FIFA PDF UK/BST",
        "utc": "2026-07-11T21:00:00Z",
        "bst": "22:00",
        "date": "2026-07-11",
        "venue": "Miami Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 100,
        "dateUK": "2026-07-12",
        "timeUK": "02:00",
        "homeCode": "W95",
        "awayCode": "W96",
        "home": "W95",
        "away": "W96",
        "stage": "Quarter-final",
        "group": "—",
        "stadium": "Kansas City Stadium",
        "city": "Kansas City",
        "source": "FIFA PDF UK/BST",
        "utc": "2026-07-12T01:00:00Z",
        "bst": "02:00",
        "date": "2026-07-12",
        "venue": "Kansas City Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 101,
        "dateUK": "2026-07-14",
        "timeUK": "20:00",
        "homeCode": "W97",
        "awayCode": "W98",
        "home": "W97",
        "away": "W98",
        "stage": "Semi-final",
        "group": "—",
        "stadium": "Dallas Stadium",
        "city": "Dallas",
        "source": "FIFA PDF UK/BST",
        "utc": "2026-07-14T19:00:00Z",
        "bst": "20:00",
        "date": "2026-07-14",
        "venue": "Dallas Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 102,
        "dateUK": "2026-07-15",
        "timeUK": "20:00",
        "homeCode": "W99",
        "awayCode": "W100",
        "home": "W99",
        "away": "W100",
        "stage": "Semi-final",
        "group": "—",
        "stadium": "Atlanta Stadium",
        "city": "Atlanta",
        "source": "FIFA PDF UK/BST",
        "utc": "2026-07-15T19:00:00Z",
        "bst": "20:00",
        "date": "2026-07-15",
        "venue": "Atlanta Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 103,
        "dateUK": "2026-07-18",
        "timeUK": "22:00",
        "homeCode": "RU101",
        "awayCode": "RU102",
        "home": "RU101",
        "away": "RU102",
        "stage": "Play-off for third place",
        "group": "—",
        "stadium": "Miami Stadium",
        "city": "Miami",
        "source": "FIFA PDF UK/BST",
        "utc": "2026-07-18T21:00:00Z",
        "bst": "22:00",
        "date": "2026-07-18",
        "venue": "Miami Stadium",
        "ukBroadcaster": ""
    },
    {
        "id": 104,
        "dateUK": "2026-07-19",
        "timeUK": "20:00",
        "homeCode": "W101",
        "awayCode": "W102",
        "home": "W101",
        "away": "W102",
        "stage": "Final",
        "group": "—",
        "stadium": "New York/New Jersey Stadium",
        "city": "New Jersey",
        "source": "FIFA PDF UK/BST",
        "utc": "2026-07-19T19:00:00Z",
        "bst": "20:00",
        "date": "2026-07-19",
        "venue": "New York/New Jersey Stadium",
        "ukBroadcaster": ""
    }
];
  var FIXTURES = SCHEDULE;
  var TV_CHANNELS = { GB:['BBC','ITV'], US:['FOX','FS1','Telemundo','Peacock'], CA:['CTV','TSN','RDS'], AU:['SBS'], IR:['IRIB TV3','Varzesh TV'] };
  function flagUrl(team){ var c=FLAG[team]||'un'; return c.indexOf('gb-')===0 ? 'https://flagcdn.com/w80/'+c+'.png' : 'https://flagcdn.com/w80/'+c+'.png'; }
  function flagImg(team, cls){ return '<img class="'+(cls||'gc-flag')+'" src="'+flagUrl(team)+'" alt="'+String(team).replace(/"/g,'&quot;')+'" loading="lazy">'; }
  function getUserTV(country){ return TV_CHANNELS[country||'GB'] || TV_CHANNELS.GB; }
  function isLive(m){ return m && m.status && ['1H','2H','HT','ET','BT','P','LIVE'].indexOf(String(m.status).toUpperCase())>-1; }
  function buildStandings(){
    var table={}; Object.keys(GROUPS).forEach(function(g){table[g]={}; GROUPS[g].forEach(function(t){table[g][t]={team:t,P:0,W:0,D:0,L:0,GF:0,GA:0,GD:0,Pts:0};});});
    SCHEDULE.forEach(function(m){
      if(!m.group || !table[m.group] || m.status!=='FT' || m.homeScore==null || m.awayScore==null) return;
      var h=table[m.group][m.home], a=table[m.group][m.away]; if(!h||!a)return;
      h.P++;a.P++;h.GF+=m.homeScore;h.GA+=m.awayScore;a.GF+=m.awayScore;a.GA+=m.homeScore;h.GD=h.GF-h.GA;a.GD=a.GF-a.GA;
      if(m.homeScore>m.awayScore){h.W++;a.L++;h.Pts+=3;} else if(m.homeScore<m.awayScore){a.W++;h.L++;a.Pts+=3;} else {h.D++;a.D++;h.Pts++;a.Pts++;}
    });
    return table;
  }
  function validate(){ return { ok:true, fixtureCount:SCHEDULE.length, source:'FIFA PDF UK/BST' }; }
  window.WC26 = {
    lastUpdated:'2026-06-12', source:'FIFA PDFs supplied by site owner; live API may override status', groups:GROUPS, fixtures:FIXTURES, schedule:SCHEDULE, standings:buildStandings(), flags:FLAG, tv:TV_CHANNELS, getUserTV:getUserTV, isLive:isLive, flagUrl:flagUrl, flagImg:flagImg, validate:validate
  };
}());
